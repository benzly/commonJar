 ...
 * other than the default. For {@link Gson} with default configuration, it is simpler to use
 * {@code new Gson()}. {@code GsonBuilder} is best used by creating it, and then invoking its
 ...
 * time-zone information. So, if you are using date/time instances, use {@code GsonBuilder} and its
 * {@code setDateFormat} methods.</li>
 * </ul>
 * </p>
 *
...
public final class GsonBuilder {
     ...
     * href="http://code.google.com/p/google-gson/issues/detail?id=42">Gson Issue 42</a> for
     * details.
     *
     * @return a reference to this {@code GsonBuilder} object to fulfill the "Builder" pattern
     * @since 1.3
     ...
     * of map serialization uses {@code toString()} on the key; however, when this is called then
     * one of the following cases apply:
     *
     * <h3>Maps as JSON objects</h3> For this case, assume that a type adapter is registered to
     * serialize and deserialize some {@code Point} class, which contains an x and y coordinate,
     * to/from the JSON Primitive string value {@code "(x,y)"}. The Java map would then be
     * serialized as a {@link JsonObject}.
     *
     * <p>
     * Below is an example:
     * 
   ...
   * }</pre>
     ...
     * For this case, assume that a type adapter was NOT registered for some {@code Point} class,
     * but rather the default Gson serialization is applied. In this case, some
     * {@code new Point(2,3)} would serialize as {@code {"x":2,"y":5}}.
     *
     * <p>
     * Given the assumption above, a {@code Map<Point, String>} will be serialize as an array of
   ...
   * }</pre>
     ...
     * means that if one of the {@code strategies} suggests that a field (or class) should be
    ...
    /**
     * Configures Gson to apply the passed in exclusion strategy during serialization. If this
     * method is invoked numerous times with different exclusion strategy objects then the exclusion
     * strategies that were added will be applied as a disjunction rule. This means that if one of
     * the added exclusion strategies suggests that a field (or class) should be skipped then that
     * field (or object) is skipped during its serialization.
     *
     ...
     * strategies that were added will be applied as a disjunction rule. This means that if one of
     * the added exclusion strategies suggests that a field (or class) should be skipped then that
     * field (or object) is skipped during its deserialization.
     *
    ...
    /**
     * By default, Gson escapes HTML characters such as < > etc. Use this option to configure Gson
     ...
     *
     * <p>
     * The date format will be used to serialize and deserialize {@link java.util.Date},
     * {@link java.sql.Timestamp} and {@link java.sql.Date}.
     *
     * <p>
     ...
     * registration of an {@link TypeAdapter}, {@link InstanceCreator}, {@link JsonSerializer}, and
     * a {@link JsonDeserializer}. It is best used when a single object {@code typeAdapter}
     * implements all the required interfaces for custom serialization with Gson. If a type adapter
     * was previously registered for the specified {@code type}, it is overwritten.
     *
     * <p>
     * This registers the type specified and no other types: you must manually register related
     * types! For example, applications registering {@code boolean.class} should also register
     * {@code Boolean.class}.
     *
    ...
    public GsonBuilder registerTypeAdapter(Type type, Object typeAdapter) {
        Preconditions.checkArgument(typeAdapter instanceof JsonSerializer<?> || typeAdapter instanceof JsonDeserializer<?>
    ...
    /**
     * Configures Gson for custom serialization or deserialization for an inheritance type
     * hierarchy. This method combines the registration of a {@link TypeAdapter},
     * {@link JsonSerializer} and a {@link JsonDeserializer}. If a type adapter was previously
     * registered for the specified type hierarchy, it is overridden. If a type adapter is
     ...
     * specification</a> (see section 4.3.20, 4.3.22, 4.3.23) allows these values as valid
     ...
     *
     * <p>
     * Gson always accepts these special values during deserialization. However, it outputs strictly
     * compliant JSON. Hence, if it encounters a float value {@link Float#NaN},
    ...
    public Gson create() {
        ...
        return new Gson(excluder, fieldNamingPolicy, instanceCreators, serializeNulls, complexMapKeySerialization,
                generateNonExecutableJson, escapeHtmlChars, prettyPrinting, serializeSpecialFloatingPointValues, longSerializationPolicy,
                factories);
    }
