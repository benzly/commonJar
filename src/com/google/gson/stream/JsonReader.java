 ...
 *
 * <h3>Parsing JSON</h3> To create a recursive descent parser for your own JSON streams, first
 * create an entry point method that creates a {@code JsonReader}.
 *
 * <p>
 * Next, create handler methods for each structure in your JSON text. You'll need a method for each
 * object type and for each array type.
 * <ul>
 * <li>Within <strong>array handling</strong> methods, first call {@link #beginArray} to consume the
 * array's opening bracket. Then create a while loop that accumulates values, terminating when
 * {@link #hasNext} is false. Finally, read the array's closing bracket by calling {@link #endArray}.
 * <li>Within <strong>object handling</strong> methods, first call {@link #beginObject} to consume
 * the object's opening brace. Then create a while loop that assigns values to local variables based
 * on their name. This loop should terminate when {@link #hasNext} is false. Finally, read the
 ...
 * For example, both elements of the JSON array {@code [1, "1"]} may be read using either
 * {@link #nextInt} or {@link #nextString}. This behavior is intended to prevent lossy numeric
 * conversions: double is JavaScript's only numeric type and very large values like
 * {@code 9007199254740993} cannot be represented exactly on that platform. To minimize precision
 * loss, extremely large values should be written and read as strings in JSON.
 *
 ...
 *
 * <p>
 * Each {@code JsonReader} may be used to read a single JSON stream. Instances of this class are not
 * thread safe.
 *
 * @author Jesse Wilson
 * @since 1.6
...
public class JsonReader implements Closeable {
     ...
     * strings without an intermediate StringBuilder. We decode literals directly out of this
     * buffer, so it must be at least as long as the longest token that can be reported as a number.
     */
    ...
    /**
     * The type of the next token to be returned by {@link #peek} and {@link #advance}. If null,
     * peek() will assign a value.
     */
    ...
    /*
     * For the next literal value, we may have the text value, or the position and length in the
     * buffer.
     */
     ...
     * <li>Top-level values of any type. With strict parsing, the top-level value must be an object
     * or an array.
    ...
    private void expect(JsonToken expected) throws IOException {
        peek();
        if (token != expected) {
            throw new IllegalStateException("Expected " + expected + " but was " + peek() + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        advance();
    }
        ...
        switch (stack[stackSize - 1]) {
            case EMPTY_DOCUMENT:
                ...
                if (!lenient && token != JsonToken.BEGIN_ARRAY && token != JsonToken.BEGIN_OBJECT) {
                    throw new IOException("Expected JSON document to start with '[' or '{' but was " + token + " at line "
    ...
    public String nextName() throws IOException {
        ...
        if (token != JsonToken.NAME) {
            throw new IllegalStateException("Expected a name but was " + peek() + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        String result = name;
        advance();
    ...
    /**
     * Returns the {@link JsonToken#STRING string} value of the next token, consuming it. If the
     * next token is a number, this method will return its string form.
     *
    ...
    public String nextString() throws IOException {
        ...
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected a string but was " + peek() + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
    ...
    public boolean nextBoolean() throws IOException {
        ...
        if (token != JsonToken.BOOLEAN) {
            throw new IllegalStateException("Expected a boolean but was " + token + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
    ...
    public void nextNull() throws IOException {
        ...
        if (token != JsonToken.NULL) {
            throw new IllegalStateException("Expected null but was " + token + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
    ...
    /**
     * Returns the {@link JsonToken#NUMBER double} value of the next token, consuming it. If the
     * next token is a string, this method will attempt to parse it as a double using
    ...
    public double nextDouble() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected a double but was " + token + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        ...
        if ((result >= 1.0d && value.startsWith("0"))) {
            throw new MalformedJsonException("JSON forbids octal prefixes: " + value + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        if (!lenient && (Double.isNaN(result) || Double.isInfinite(result))) {
            throw new MalformedJsonException("JSON forbids NaN and infinities: " + value + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
     ...
     * token is a string, this method will attempt to parse it as a long. If the next token's
     * numeric value cannot be exactly represented by a Java {@code long}, this method throws.
     *
     * @throws IllegalStateException if the next token is not a literal value.
    ...
    public long nextLong() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected a long but was " + token + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        ...
        try {
            ...
            if (result != asDouble) {
                throw new NumberFormatException("Expected a long but was " + value + " at line " + getLineNumber() + " column "
                        + getColumnNumber());
            }
        }
     ...
     * token is a string, this method will attempt to parse it as an int. If the next token's
    ...
    public int nextInt() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected an int but was " + token + " at line " + getLineNumber() + " column "
                    + getColumnNumber());
        }
        ...
        try {
            ...
            result = (int) asDouble;
            if (result != asDouble) {
                throw new NumberFormatException("Expected an int but was " + value + " at line " + getLineNumber() + " column "
                        + getColumnNumber());
            }
        }
    ...
    /**
     * Skips the next value recursively. If it is an object or array, all nested elements are
     * skipped. This method is intended for use when the JSON token stream contains unrecognized or
     * unhandled values.
     */
    ...
    private JsonToken nextInObject(boolean firstElement) throws IOException {
        /*
         * Read delimiters. Either a comma/semicolon separating this and the previous name-value
         * pair, or a close brace to denote the end of the object.
         */
        if (firstElement) {
    ...
    private JsonToken objectValue() throws IOException {
        /*
         * Read the name/value separator. Usually a colon ':'. In lenient mode we also accept an
    ...
    private int nextNonWhitespace(boolean throwOnEof) throws IOException {
        /*
         * This code uses ugly local variables 'p' and 'l' representing the 'pos' and 'limit' fields
         * respectively. Using locals rather than fields saves a few field reads for each whitespace
         * character in a pretty-printed document, resulting in a 5% speedup. We need to flush 'p'
         * to its field before any (potentially indirect) call to fillBuffer() and reread both 'p'
         * and 'l' after any (potentially indirect) call to the same method.
         */
        char[] buffer = this.buffer;
        int p = pos;
        int l = limit;
     ...
     *        and return a null result. This only works if the literal is short; a string is
     *        returned otherwise.
     */
    @SuppressWarnings("fallthrough")
    private String nextLiteral(boolean assignOffsetsOnly) throws IOException {
        ...
        findNonLiteralCharacter: while (true) {
            ...
            /*
             * Attempt to load the entire literal into the buffer at once. If we run out of input,
             * add a non-literal character at the end so that decoding doesn't need to do bounds
             * checks.
             */
            if (i < buffer.length) {
                if (fillBuffer(i + 1)) {
    ...
    /**
     * Determine whether the characters is a JSON number. Numbers are of the form -12.34e+56.
     * Fractional and exponential parts are optional. Leading zeroes are not allowed in the value or
     * exponential part, but are allowed in the fraction.
     */
        ...
        JsonReaderInternalAccess.INSTANCE = new JsonReaderInternalAccess() {
            ...
            public void promoteNameToValue(JsonReader reader) throws IOException {
                ...
                if (reader.token != JsonToken.NAME) {
                    throw new IllegalStateException("Expected a name but was " + reader.peek() + " " + " at line " + reader.getLineNumber()
