 ...
 *
 * <p>
 * You can create a Gson instance by invoking {@code new Gson()} if the default configuration is all
 * you need. You can also use {@link GsonBuilder} to build a Gson instance with various
 * configuration options such as versioning support, pretty printing, custom {@link JsonSerializer}
 * s, {@link JsonDeserializer}s, and {@link InstanceCreator}s.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * If the object that your are serializing/deserializing is a {@code ParameterizedType} (i.e.
 * contains at least one type parameter and may be an array) then you must use the
 * {@link #toJson(Object, Type)} or {@link #fromJson(String, Type)} method. Here is an example for
 ...
 *
 * <p>
 * See the <a href="https://sites.google.com/site/gson/gson-user-guide">Gson User Guide</a> for a
...
public final class Gson {
     ...
     * as is since an array is an ordered list. Moreover, if a field is not null, but its generated
     * JSON is empty, the field is kept. You can configure Gson to serialize null values by setting
     ...
     * invoking {@link GsonBuilder#setDateFormat(int)} or {@link GsonBuilder#setDateFormat(String)}.
     * </li>
     ...
     * field <code>versionNumber</code> will be output as <code>"versionNumber@quot;</code> in Json.
     * The same rules are applied for mapping incoming Json to the Java classes. You can change this
     * policy through {@link GsonBuilder#setFieldNamingPolicy(FieldNamingPolicy)}.</li>
     ...
     */
    public Gson() {
        this(Excluder.DEFAULT, FieldNamingPolicy.IDENTITY, Collections.<Type, InstanceCreator<?>>emptyMap(), false, false,
                DEFAULT_JSON_NON_EXECUTABLE, true, false, false, LongSerializationPolicy.DEFAULT, Collections
                        .<TypeAdapterFactory>emptyList());
    }

    Gson(final Excluder excluder, final FieldNamingStrategy fieldNamingPolicy, final Map<Type, InstanceCreator<?>> instanceCreators,
            boolean serializeNulls, boolean complexMapKeySerialization, boolean generateNonExecutableGson, boolean htmlSafe,
            boolean prettyPrinting, boolean serializeSpecialFloatingPointValues, LongSerializationPolicy longSerializationPolicy,
     ...
     * registered. This features is typically used when you want to register a type adapter that
     * does a little bit of work but then delegates further processing to the Gson default type
     * adapter. Here is an example:
     * <p>
     * Let's say we want to write a type adapter that counts the number of objects being read from
     * or written to JSON. We can achieve this by writing a type adapter factory that uses the
   ...
   *  }</pre>
     ...
     * types, our stats factory will not count the number of String or primitives that will be read
     * or written.
     * 
     * @param skipPast The type adapter factory that needs to be skipped while searching for a
     *        matching type adapter. In most cases, you should just pass <i>this</i> (the type
     *        adapter factory from where {@link #getDelegateAdapter} method is being invoked).
     ...
     * of Java. Note that this method works fine if the any of the object fields are of generic
     * type, just the object itself should not be of a generic type. If the object is of generic
     * type, use {@link #toJsonTree(Object, Type)} instead.
     *
   ...
   * </pre>
     ...
     * itself should not be of a generic type. If the object is of generic type, use
   ...
   * </pre>
    ...
    /**
     * Returns a new JSON writer configured for this GSON and with the non-execute prefix if that is
     * configured.
     */
     ...
     * a generic type. For the cases when the object is of generic type, invoke
     * {@link #fromJson(String, Type)}. If you have the Json in a {@link Reader} instead of a
     * String, use {@link #fromJson(Reader, Class)} instead.
     *
   ...
   * </pre>
     ...
     * this method works fine if the any of the fields of the specified object are generics, just
     * the object itself should not be a generic type. For the cases when the object is of generic
     * type, invoke {@link #fromJson(Reader, Type)}. If you have the Json in a String form instead
     * of a {@link Reader}, use {@link #fromJson(String, Class)} instead.
     *
     * @param <T> the type of the desired object
   ...
   * </pre>
    ...
    /**
     * Reads the next JSON value from {@code reader} and convert it to an object of type
     * {@code typeOfT}. Since Type is not parameterized by T, this method is type unsafe and should
     * be used carefully
     *
     ...
     * this method works fine if the any of the fields of the specified object are generics, just
     * the object itself should not be a generic type. For the cases when the object is of generic
     * type, invoke {@link #fromJson(JsonElement, Type)}.
     * 
     * @param <T> the type of the desired object
   ...
   * </pre>
    ...
    public String toString() {
        StringBuilder sb =
                new StringBuilder("{").append("serializeNulls:").append(serializeNulls).append("factories:").append(factories)
                        .append(",instanceCreators:").append(constructorConstructor).append("}");
        return sb.toString();
    }
}
